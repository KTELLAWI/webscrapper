...
function SearchPage({params : {id} } : Props) {
    ...
    if(loading) 
    return(
        <h1 className='text-center p-10 animate-pulse text-xl text-blue-600'>جاري تحميل النتائج...</h1>
    )
        ...
        <div className='flex items-center justify-between mb-7'>
            <div className='flex flex-col md:flex-row gap-x-4'>
                ...
                <p className='text-grey-300 text-1xl'>
                    {snapshot.data()?.results?.length > 0 && `${snapshot.data()?.results?.length} نتيجة تم ايجادها `}
                </p>
